...
def get_children(diag, node):
  ...
  if (node[0] > 0): # direction 0
    ...
    if is_walkable(diag[child[0]][child[1]]) and is_connected(node, child, 0):
      children[0] = child
  if (node[1] < len(diag[0]) - 1): # direction 1
...
print("Now looking for interior tiles")
...
for line in range(len(diagram)):
  stack = []
  inbound = False
  for i in range(len(diagram[line])):
    c = diagram[line][i]
    r = raw[line][i]
    if c != '?':
      if len(stack) %2 == 1:
    ...
    if c == '?':
      ...
      if r in '7':
        if stack[-1] != 'L':
          stack.insert(len(stack), r)
      if r in '-':
        if not stack[-1] in 'FL':
           stack.insert(len(stack), r)
    print(f'Line {line} column {i}: {diagram[line][i]} {raw[line][i]}: {stack}')

print(f'The number of tiles within is {count}')
print_diagram(diagram)
